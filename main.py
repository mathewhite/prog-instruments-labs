...
class Wordle:
    ...
    def __init__(self):
        ...
        self.width = 600
        self.height = 800
        self.x_co = (int(self.root.winfo_screenwidth() / 2)
                     - int(self.width / 2))
        self.y_co = 50
        ...
        self.setting_dark = self.setting_dark.resize(
        ...
        self.b_row1 = self.b_row2 = self.b_row3 =\
            self.b_row4 = self.b_row5 = self.b_row6 = []
        self.buttons = []
        ...
        if e:
            if e.keysym == "BackSpace":
                ...
                self.buttons[self.current_B_row][self.current_b]["text"]\
                    = key.upper()
                self.buttons[self.current_B_row][self.current_b]['bg']\
                    = "#3d3d3d"
            ...
            if self.word_api.is_valid_guess(self.guess):
                ...
                for i in range(self.word_size):
                    if self.word_api.is_at_right_position(i, self.guess[i]):
                        ...
                        self.change_keypad_color("#4d4a4a", self.guess[i],
                                    "#3d3b3b", "#4d4a4a")
    ...
    def get_from_db(self):
        if not os.path.exists("settings.db"):
            ...
            cursor.execute("CREATE TABLE info(id integer,"
                           " word_length integer,high_score integer)")
